...
| 30 | Página `/recetas` (admin): tarjetas por familia de servicio con tallas en columnas y edición en línea de gramos, insumos y horas; tab Insumos con envase editable, costo por gramo y servicios que usan cada producto; tab Rentabilidad (real por mes: libro con ingresos, insumos por foto, comisiones devengadas, margen de productos y fijos vigentes del mes con veredicto de punto de equilibrio; servicios y comisión por profesional; proyección con el promedio de los 3 meses cerrados anteriores y suba de precio sugerida y aplicable sobre los servicios de margen bajo). Profesionales asignadas por servicio con % propio (`hairdresser_services`, mig. `093`): matriz por familia en Ajustes › Operaciones (reemplaza los presets `commission_rates`), un toque en Carga Rápida carga la asignada con su %, propuesta en Transacciones, equipo simulado en Rentabilidad. `products_with_stock` cae al costo del último lote cuando no hay stock (mig. `091`, restaura el fallback perdido en `026`) y backfill de las fotos de `transaction_recipe_costs` que quedaron en 0 (mig. `092`). Recetas actualizadas desde el Excel de costos (datos, sin migración) |
