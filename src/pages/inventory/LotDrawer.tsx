...
export function LotDrawer({ product, onClose }: LotDrawerProps) {
  ...
  async function save(lot: InventoryLot, field: EditingCell['field'], rawValue: string) {
    ...
    if (field === 'remaining_quantity') {
      setPendingAdjustment({ lot, newQty: parsed as number })
      setAdjustmentReason('')
      return
    }
  ...
  async function confirmAdjustment() {
    ...
    const { lot, newQty } = pendingAdjustment
    const delta = newQty - Number(lot.remaining_quantity)
    setSavingId(lot.id)
    try {
      ...
      await updateLot.mutateAsync({ id: lot.id, remaining_quantity: newQty })
    } finally {
      setSavingId(null)
                    ...
                    <td className="py-2 pr-4 text-right tabular-nums">
                      {lot.has_sales ? (
                        <span
                          ...
                          title="No editable: este lote ya tiene ventas registradas"
                        >
          ...
          <div className="space-y-4">
            <p className="text-sm text-[var(--color-muted)]">
              ...
              <span className="font-medium text-[var(--color-text)]">
                {Number(pendingAdjustment.lot.remaining_quantity).toLocaleString('es-CO')}
              </span>
              {' → '}
              <span className="font-medium text-[var(--color-text)]">
                {Number(pendingAdjustment.newQty).toLocaleString('es-CO')}
              </span>
              {' '}
              <span className={pendingAdjustment.newQty - Number(pendingAdjustment.lot.remaining_quantity) >= 0 ? 'text-green-600' : 'text-red-500'}>
                ({pendingAdjustment.newQty - Number(pendingAdjustment.lot.remaining_quantity) >= 0 ? '+' : ''}
                {(pendingAdjustment.newQty - Number(pendingAdjustment.lot.remaining_quantity)).toLocaleString('es-CO')})
              </span>
            </p>
            <Input
            ...
            />
            <div className="flex justify-end gap-2">
              <Button variant="secondary" size="sm" onClick={() => setPendingAdjustment(null)}>Cancelar</Button>
              <Button
                size="sm"
                onClick={confirmAdjustment}
                loading={createMovement.isPending || updateLot.isPending}
              >
                Confirmar ajuste
              </Button>
            </div>
