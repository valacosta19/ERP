...
BEGIN
  SELECT status, shipping_cost, discount_amount
  INTO current_status, v_shipping_cost, v_discount_amount
  ...
  LOOP
    ...
    IF v_total_items_cost > 0 THEN
      effective_unit_cost := item.unit_cost
        + (v_shipping_cost * (v_received_qty * item.unit_cost) / v_total_items_cost) / v_received_qty
        - (v_discount_amount * (v_received_qty * item.unit_cost) / v_total_items_cost) / v_received_qty;
    ELSE
      effective_unit_cost := item.unit_cost;
    END IF;
