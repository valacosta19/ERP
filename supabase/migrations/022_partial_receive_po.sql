-- Adds optional p_items parameter to receive_purchase_order.
-- p_items: JSONB array of {id: uuid, quantity: numeric} — one entry per
-- purchase_order_item to receive. Items omitted or with quantity <= 0 are skipped.
-- Shipping cost is distributed proportionally among actually-received items only.
-- When p_items is NULL, all items are received at their ordered quantities (backward-compatible).
