-- Add client_uuid to transactions for idempotent funnel submissions
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS client_uuid uuid;

CREATE UNIQUE INDEX IF NOT EXISTS transactions_client_uuid_key
  ON transactions(client_uuid)
  WHERE client_uuid IS NOT NULL;

-- Atomic, idempotent RPC for QuickFunnel ticket unit submission
