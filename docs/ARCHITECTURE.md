...
**Qué hace:** Página admin con tres tabs: **Recetas** (una tarjeta por familia de servicio con las tallas corto/mediano/largo en columnas, gramos y costo por insumo, total de materiales y % sobre precio; cada tarjeta tiene modo edición para gramos por talla, alta/baja de insumos y horas, con guardado por talla), **Insumos** (productos usados en alguna receta: envase editable en línea, último costo, costo por gramo, stock y servicios que lo usan, expandible; cada servicio abre su familia en edición) y **Rentabilidad** (datos reales, sin simulación: se elige un mes y una ganancia objetivo mensual; libro del mes con ingresos por servicios − insumos − comisiones devengadas = margen de servicios, + margen de productos, − fijos del mes, = resultado, con veredicto y barra de avance hacia el punto de equilibrio; tabla por profesional con servicios hechos, ingreso y comisión, expandible por servicio; proyección con el promedio real de los 3 meses cerrados anteriores contra los mismos fijos; y tabla por servicio con promedios reales, margen bruto, precio actual y precio sugerido con "Aplicar" y "Aplicar a todos los sugeridos").
...
- `src/lib/profitability.ts` — `marginColor` (con tests)
- `src/lib/realProfitability.ts` — `buildServiceSales`, `sumSales`, `summarizeByService`, `summarizeByProfessional`, `fixedCostsForMonth`, `monthResult`, `priceRaiseSuggestion`, `raisedPrice` (con tests)
- `src/lib/dateRange.ts` — `monthRange(ym)`, `previousMonthsRange(ym, n)` (con tests)
- `src/hooks/useServiceRecipes.ts` — `useAllServiceRecipes` (clave `['service-recipes-all']`, paginado con `fetchAllRows`)
- `src/hooks/useServiceSalesDetail.ts` — `useServiceSalesDetail(from, to)`: transacciones de ingreso de servicio en un rango con sus `transaction_hairdressers` (`!inner` + `fetchAllRows`, con tests)
- `src/hooks/useDolarBlue.ts` — cotización blue (`['dolar-blue']`, 30 min), compartida con Reportes

**Datos:** `catalog_items`, `service_recipes`, `fixed_costs`, `fixed_cost_rates`, `transactions`, `transaction_hairdressers`, `transaction_recipe_costs`, `sale_items` (vía `useProfitReport`), vista `products_with_stock`. La edición del envase usa `useUpdateProduct` (mismo campo `unit_size` que Inventario); "Aplicar" sube `price`, `price_transfer` y `price_card` en la misma proporción con `useUpdateCatalogItem` y `confirmDialog`.

**Fórmulas (Rentabilidad):** por cada venta de servicio (ingreso, no seña, no anulada, EUR excluido, USD al blue de hoy): ingreso = `amount + seña_amount`; comisión devengada = ingreso × Σ `commission_rate` de sus `transaction_hairdressers`; insumos = foto de `transaction_recipe_costs` (Σ gramos × costo ÷ envase) o, sin foto, receta actual × `getCostPerGram`. Margen de servicios = ingreso − insumos − comisión. Fijos del mes = por cada `fixed_costs` activo, la tarifa de `fixed_cost_rates` con `effective_from` ≤ fin de mes más reciente (sin tarifa, `monthly_amount`). Resultado = margen de servicios + `product_profit` del mes − fijos − ganancia objetivo; hueco = max(0, −resultado). Proyección = lo mismo con el promedio mensual de los 3 meses cerrados anteriores al mes elegido. Sugerencia de suba: umbral = margen bruto % promedio ponderado por ingreso mensual; se suben los servicios con margen < umbral (si ninguno o sin ventas, todos) con un % único = hueco ÷ Σ (servicios/mes × ingreso promedio × (1 − comisión %)); precio sugerido = precio × (1 + %) redondeado a 100.
...
- **Rentabilidad mide punto de equilibrio, no utilidad contable.** Resta los fijos configurados del mes completo (aunque el mes esté en curso) y las comisiones devengadas, para responder "¿ya cubrí los fijos?". Reportes › Utilidad resta los gastos realmente cargados en el período y las comisiones solo cuando se pagan. Son dos preguntas distintas y no deben unificarse ni cuadrarse entre sí. Los fijos nunca se descuentan por servicio: la tabla por servicio muestra solo margen bruto.
- **La sugerencia de precios no persiste nada hasta "Aplicar".** El % de suba y la ganancia objetivo viven en el estado de la vista; "Aplicar" escribe en `catalog_items` los tres precios escalados.
