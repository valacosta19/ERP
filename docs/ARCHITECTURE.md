...
- **Los filtros de la lista viven en la query string** (`cat`, `cur`, `method`, `from`, `to`, `voided`, `pending`); se omite el parámetro cuando vale el default (mes actual, todo, apagado) y un valor inválido cae al default. Selección y expansión siguen en estado React.
