...
**Qué hace:** Página admin con tres tabs: **Recetas** (una tarjeta por familia de servicio con las tallas corto/mediano/largo en columnas, gramos y costo por insumo, total de materiales y % sobre precio; cada tarjeta tiene modo edición para gramos por talla, alta/baja de insumos y horas, con guardado por talla), **Insumos** (productos usados en alguna receta: envase editable en línea, último costo, costo por gramo, stock y servicios que lo usan, expandible; cada servicio abre su familia en edición) y **Rentabilidad** (simulador: precio por método de cobro, equipo de profesionales marcadas cuyo % se suma por servicio asignado, costo fijo por hora = fijos activos ÷ horas/mes editables como referencia, margen objetivo; por servicio muestra insumos, comisión, margen bruto, precio sugerido con "Aplicar", servicios/mes y margen mensual proyectado, con barra de composición del precio y desglose expandible; al pie de la tabla, cierre del mes: margen bruto proyectado − fijos del mes = resultado neto estimado).
...
**Fórmulas (Rentabilidad):** comisión = precio × % donde % es la suma de los % de las profesionales marcadas en el "equipo" del panel que tienen el servicio asignado (`hairdresser_services`; por defecto la de más asignaciones); si ninguna marcada lo hace, el servicio queda sin margen, sin sugerido y fuera de promedios, proyección y cierre mensual; margen bruto = precio − insumos − comisión; precio sugerido = insumos ÷ (1 − comisión% − objetivo%), redondeado a 100, nulo si comisión + objetivo ≥ 100 %. Servicios/mes = transacciones de los últimos 3 meses cerrados ÷ 3. Pie de tabla: margen bruto proyectado (Σ margen bruto × servicios/mes) − Σ fijos activos del mes = resultado neto estimado, con veredicto rentable / no rentable; sin ventas registradas no se estima.
...
- **El simulador de Rentabilidad no persiste el escenario ni usa historial.** Comisión, horas/mes y margen objetivo viven en el estado de la vista; el costo fijo por hora (fijos activos de hoy ÷ horas/mes) es solo una referencia del panel y no entra en ningún margen: los fijos se descuentan por período en Reportes › Utilidad, nunca por servicio. Reportes › Costos sigue siendo el margen bruto histórico con la comisión real cobrada: son dos preguntas distintas y no deben unificarse.
